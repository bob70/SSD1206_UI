...
int main(void) {
  ...
  OledWidget *screenWidget = new OledWidget(128, 64, 3, false, &display);
  GaugeWidget *gaugeWidget = new GaugeWidget(10, 50, 0, false, &display);

  screenWidget->addChild(gaugeWidget, 10, 10);

  while (1) {
    //printf("loop (wheel cpt : %d)\n", wheelCpt);


    gaugeWidget->setValue(wheelCpt / 100);
    if (screenWidget->draw()) {
      display.clearDisplay();
      display.display();
    }
    /*
...
static void MX_GPIO_Init(void) {
  ...
  // Configure GPIO pins : ENCOD_LEFT_Pin
  GPIO_InitStruct.Pin = ENCOD_LEFT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING; // interrupt for encoding wheel
  GPIO_InitStruct.Pull = GPIO_PULLUP;;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  // Configure GPIO pins : ENCOD_RIGHT_Pin
  GPIO_InitStruct.Pin = ENCOD_RIGHT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT; // just an input to check the "right" direction
  GPIO_InitStruct.Pull = GPIO_PULLUP;;
  HAL_GPIO_Init(ENCOD_RIGHT_GPIO_Port, &GPIO_InitStruct);

  // EXTI interrupt init (for ENCOD_LEFT_Pin and ENCOD_RIGHT_Pin)
